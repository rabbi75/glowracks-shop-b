...
@section('title','Highlight Manage')
      ...
      <div class="row">
        <div class="col-12">
          <div class="card">
            ...
            <h3 class="card-title">Manage Highlight information</h3>
      			<div class="short_button">
              <a href="{{url('editor/highlight/add')}}"><i class="fa fa-plus"></i>Add</a>
      			</div>
          </div>
          <!-- /.card-header -->
            <div class="card-body user-border">
              <table id="example" class="table table-responsive-sm table-bordered table-striped">
                ...
                <tr>
                  <td>{{ $value->serial }}</td>
                  <td>{{ $value->desc }}</td>
                  <td><img src="{{asset($value->image)}}" style="width: auto; height: 40px; display: block; margin: 0 auto;" alt=""></td>
                  ...
                  <td>
                    <ul class="action_buttons dropdown">
                        ...
                        <ul class="dropdown-menu">
                          <li>
                        @if($value->status==1)
                        <form action="{{url('editor/highlight/inactive')}}" method="POST">
                          @csrf
                          <input type="hidden" name="hidden_id" value="{{$value->id}}">
                          <button type="submit" class="thumbs_up" title="unpublished"><i class="fa fa-thumbs-up"></i> Active</button> 
                        </form>
                        @else
                          <form action="{{url('editor/highlight/active')}}" method="POST">
                            @csrf
                            <input type="hidden" name="hidden_id" value="{{$value->id}}">
                        ...
                        @endif
                          ...
                          <li>
                        <a class="edit_icon" href="{{url('editor/highlight/edit/'.$value->id)}}" title="Edit"><i class="fa fa-edit"></i> Edit</a>
                      </li>
                      <li>
                        <form action="{{url('editor/highlight/delete')}}" method="POST">
                            @csrf
                            <input type="hidden" name="hidden_id" value="{{$value->id}}">
